...
def book_left(book_id):
    book_total = 0
    book_borrowed = 0
    book_id_str = book_id
    for user in borrow_info.keys():
        book_borrowed += borrow_info[user][book_id_str]
    for book in books:
        if book['book_id'] == book_id_str:
            book_total = book['number']
    return book_total - book_borrowed


def borrow_return_book(user, book_id, number):
    # 借出 number 为正数，还书 number 为负数
    global borrow_info
    if book_id in borrow_info[user].keys():
        borrow_info[user][book_id] += number
    else:
        borrow_info[user][book_id] = number


def client_borrow_book(parameter_list, address_client):
    """
        计算借书后剩余的书，返回的负值，代表缺少几本书，借书失败；非负值代表剩余几本书，借书成功
    """
    user = parameter_list[0].decode('utf-8')
    book_id = parameter_list[1].decode('utf-8')
    number = int(parameter_list[2])
    book_left_int = book_left(book_id)
    if book_left_int >= number:
        borrow_return_book(user, book_id, number)
        socket_server.sendto(('Borrow_Response {}'.format(book_left_int - number)).encode('utf-8'), address_client)
    else:
        socket_server.sendto(('Borrow_Response {}'.format(book_left_int - number)).encode('utf-8'), address_client)
...
def execute_command(command_bytes, parameter_list, address_client):
    ...
    elif command_bytes == b'Borrow_Book':
        client_borrow_book(parameter_list, address_client)
